...
Param(
    ...
    [Parameter()][switch]$SkipShortcutVisualSmoke
)
...
function Convert-VisualSmokeOutput {
    ...
    $shortcutE2EVerified = $null
    $binarySubsystem = $null
    $binarySubsystemValue = $null
    ...
    if ($Mode -eq "split") {
        ...
        if ($values["shortcut_e2e_verified"] -ne "True") {
            throw "shortcut visual smoke did not verify the shortcut workflow"
        }
        if ($values["binary_subsystem"] -ne "Windows GUI") {
            throw "shortcut visual smoke requires a Windows GUI subsystem binary; actual: $($values["binary_subsystem"])"
        }

        $binarySubsystemValue = Convert-OutputInt64 -Value $values["binary_subsystem_value"] -Name "shortcut visual smoke binary_subsystem_value"
        $visibleTopLevelWindows = Convert-OutputInt64 -Value $values["visible_top_level_windows"] -Name "shortcut visual smoke visible_top_level_windows"
        if ($binarySubsystemValue -ne 2) {
            throw "shortcut visual smoke reported an unexpected PE subsystem value: $binarySubsystemValue"
        }
        ...
        foreach ($captureName in @(
            ...
            $colorsKey = "shortcut_capture_$($captureName)_sampled_unique_colors"
            $capturePath = [System.IO.Path]::GetFullPath((Get-RequiredOutputValue -Values $values -Key $pathKey -Context "shortcut visual smoke"))
            $captureBytes = Convert-OutputInt64 -Value (Get-RequiredOutputValue -Values $values -Key $bytesKey -Context "shortcut visual smoke") -Name "$captureName shortcut screenshot bytes"
            $captureUniqueColors = Convert-OutputInt64 -Value (Get-RequiredOutputValue -Values $values -Key $colorsKey -Context "shortcut visual smoke") -Name "$captureName shortcut sampled colors"

            Assert-VisualSmokeFile -Path $capturePath -ExpectedBytes $captureBytes -Name "shortcut screenshot $captureName"
            if ($captureUniqueColors -lt 8) {
                throw "shortcut screenshot $captureName appears blank or nearly blank"
            }

            $shortcutCaptures += [pscustomobject]@{
                name = $captureName
                file = $capturePath
                bytes = $captureBytes
        ...
        foreach ($entry in $shortcutComparisonEntries) {
            if ($entry.Value -notmatch '^different_pixel_ratio=([0-9.Ee+-]+)\s+average_channel_delta=([0-9.Ee+-]+)\s+diff=(.+)$') {
                throw "shortcut visual smoke comparison entry was not parseable: $($entry.Key): $($entry.Value)"
            }

            $comparisonName = $entry.Key.Substring("shortcut_comparison_".Length)
            $differentPixelRatio = Convert-OutputDouble -Value $Matches[1] -Name "$comparisonName shortcut different_pixel_ratio"
            $averageChannelDelta = Convert-OutputDouble -Value $Matches[2] -Name "$comparisonName shortcut average_channel_delta"
            $diffFile = [System.IO.Path]::GetFullPath($Matches[3])
            if (-not (Test-Path -LiteralPath $diffFile -PathType Leaf)) {
                throw "shortcut visual smoke comparison diff file was missing: $diffFile"
            }
            if ((Get-Item -LiteralPath $diffFile).Length -le 0) {
                throw "shortcut visual smoke comparison diff file was empty: $diffFile"
            }
            if ($differentPixelRatio -le 0 -or $averageChannelDelta -le 0) {
                throw "shortcut visual smoke comparison did not record a visible image difference for $comparisonName"
            }

            $shortcutComparisons += [pscustomobject]@{
                name = $comparisonName
...
try {
    ...
    if (-not $SkipVisualSmoke -and -not $SkipShortcutVisualSmoke) {
        Invoke-Step "shortcut binary exists" {
            if (-not (Test-Path -LiteralPath $ShortcutBinary -PathType Leaf)) {
                throw "zed-terminal shortcut smoke binary not found: $ShortcutBinary"
            }
        }
    }
    ...
    if (-not $SkipCliDiagnostics) {
        Invoke-Step "CLI diagnostics" {
            Invoke-NativeTextCommand "help-profile-transfer" @(
                ...
                "--restore-profile-mutation-backup --restore-profile-mutation-backup-file <FILE>",
                "--restore-profile-mutation-backup-format <text\|json>",
            ...
            if (
                ...
                ($sessionReportJson | ConvertTo-Json -Depth 8) -match "do-not-log-release-session-buffer-secret"
            ) {
                throw "zed-terminal --session-report did not return expected metadata-only session diagnostics"
            }
            $sessionClearJson = Invoke-NativeJsonCommandResult "clear-saved-session" @(
                "--user-data-dir", $cliDataDir,
                "--config-dir", $cliConfigDir,
                "--clear-saved-session",
            ...
            $configBundleBackupFileAction = $keymapActions.actions | Where-Object { $_.name -eq "zed_terminal::OpenConfigBundleBackupFile" } | Select-Object -First 1
            if (-not $configBundleBackupFileAction -or $configBundleBackupFileAction.namespace -ne "zed_terminal" -or $configBundleBackupFileAction.input -ne "none") {
                throw "Keymap action list is missing the OpenConfigBundleBackupFile action metadata."
            }
            ...
            $profileImportAction = $keymapActions.actions | Where-Object { $_.name -eq "zed_terminal::ImportStartupProfile" } | Select-Object -First 1
            if (-not $profileImportAction -or $profileImportAction.namespace -ne "zed_terminal" -or $profileImportAction.input -ne "none") {
                throw "Keymap action list did not report the expected ImportStartupProfile no-input metadata."
            }
            ...
            $configBundleBackupFileActionDescription = Invoke-NativeJsonCommandResult "describe-keymap-action-config-bundle-backup-file" @(
                "--user-data-dir", $cliDataDir,
                "--config-dir", $cliConfigDir,
                "--describe-keymap-action", "zed_terminal::OpenConfigBundleBackupFile",
                "--describe-keymap-action-format", "json"
            )
            if ($configBundleBackupFileActionDescription.action.name -ne "zed_terminal::OpenConfigBundleBackupFile" -or $configBundleBackupFileActionDescription.action.namespace -ne "zed_terminal" -or $configBundleBackupFileActionDescription.action.input -ne "none") {
                throw "Keymap action description did not report the expected config bundle backup file action contract."
            }
            ...
            $profileCopyActionDescription = Invoke-NativeJsonCommandResult "describe-keymap-action-profile-copy" @(
                "--user-data-dir", $cliDataDir,
                "--config-dir", $cliConfigDir,
                "--describe-keymap-action", "zed_terminal::CopyStartupProfile",
                "--describe-keymap-action-format", "json"
            )
            ...
            $profileImportActionDescription = Invoke-NativeJsonCommandResult "describe-keymap-action-profile-import" @(
                "--user-data-dir", $cliDataDir,
                "--config-dir", $cliConfigDir,
                "--describe-keymap-action", "zed_terminal::ImportStartupProfile",
                "--describe-keymap-action-format", "json"
            )
            if ($profileImportActionDescription.action.name -ne "zed_terminal::ImportStartupProfile" -or $profileImportActionDescription.action.namespace -ne "zed_terminal" -or $profileImportActionDescription.action.input -ne "none") {
                throw "Keymap action description did not report the expected profile import action contract."
            }
            ...
            $startupProfileSlotsReportActionDescription = Invoke-NativeJsonCommandResult "describe-keymap-action-startup-profile-slots-report" @(
                "--user-data-dir", $cliDataDir,
                "--config-dir", $cliConfigDir,
                "--describe-keymap-action", "zed_terminal::OpenStartupProfileSlotsReport",
                "--describe-keymap-action-format", "json"
            )
...
'@ -Encoding ascii
            ...
            $specificProfileMutationBackupFile = Join-Path $profileMutationBackupDir "terminal-before-profile-copy-profile-release-specific-41.json"
...
'@
            $specificProfileMutationBackupPayload = @'
{
  "profiles": {
    "release-specific": {
      "display_name": "Release Specific Secret Profile",
      "env": {
        "RELEASE_SPECIFIC_PROFILE_MUTATION_SECRET": "release-specific-profile-mutation-secret"
      }
    }
  }
}
'@
            ...
            Set-Content -LiteralPath $specificProfileMutationBackupFile -Value $specificProfileMutationBackupPayload -Encoding ascii -NoNewline
            $profileMutationBackupTimestamp = [DateTime]::UtcNow
            (Get-Item -LiteralPath $specificProfileMutationBackupFile).LastWriteTimeUtc = $profileMutationBackupTimestamp
            (Get-Item -LiteralPath $profileMutationBackupFile).LastWriteTimeUtc = $profileMutationBackupTimestamp.AddMinutes(5)
            ...
            $specificProfileMutationBackupEntry = $profileMutationBackupEntries | Where-Object { $_.path -eq $specificProfileMutationBackupFile } | Select-Object -First 1
            if (
                ($profileMutationBackups.status -ne "ok" -and $profileMutationBackups.status -ne "warning") -or
                [int64]$profileMutationBackups.backup_count -lt 2 -or
                [int64]$profileMutationBackups.valid_count -lt 2 -or
                -not $profileMutationBackupEntry -or
                -not $specificProfileMutationBackupEntry -or
                $profileMutationBackupEntry.valid -ne $true -or
                $specificProfileMutationBackupEntry.valid -ne $true -or
                $profileMutationBackupEntry.action -ne "copy-profile" -or
                $specificProfileMutationBackupEntry.action -ne "copy-profile" -or
                $profileMutationBackupEntry.profile_stem -ne "release-smoke" -or
                $specificProfileMutationBackupEntry.profile_stem -ne "release-specific" -or
                [int64]$profileMutationBackupEntry.profile_count -ne 1 -or
                [int64]$specificProfileMutationBackupEntry.profile_count -ne 1
            ) {
            ...
            if ($profileMutationBackupsText -match "Release Secret Profile" -or $profileMutationBackupsText -match "RELEASE_PROFILE_MUTATION_SECRET" -or $profileMutationBackupsText -match "release-profile-mutation-secret" -or $profileMutationBackupsText -match "Release Specific Secret Profile" -or $profileMutationBackupsText -match "RELEASE_SPECIFIC_PROFILE_MUTATION_SECRET" -or $profileMutationBackupsText -match "release-specific-profile-mutation-secret") {
            ...
            if ($profileMutationRestoreText -match "Release Secret Profile" -or $profileMutationRestoreText -match "RELEASE_PROFILE_MUTATION_SECRET" -or $profileMutationRestoreText -match "release-profile-mutation-secret" -or $profileMutationRestoreText -match "Release Specific Secret Profile" -or $profileMutationRestoreText -match "RELEASE_SPECIFIC_PROFILE_MUTATION_SECRET" -or $profileMutationRestoreText -match "release-specific-profile-mutation-secret") {
            ...
            Set-Content -LiteralPath $mutationStartupConfigFile -Value "{ broken terminal config for specific restore" -NoNewline
            $specificProfileMutationRestore = Invoke-NativeJsonCommandResult "mutation-restore-profile-mutation-backup" @(
                "--user-data-dir", $mutationCliDataDir,
                "--config-dir", $mutationCliConfigDir,
                "--restore-profile-mutation-backup",
                "--restore-profile-mutation-backup-file", $specificProfileMutationBackupFile,
                "--restore-profile-mutation-backup-format", "json"
            )
            if (
                $specificProfileMutationRestore.status -ne "ok" -or
                $specificProfileMutationRestore.startup_config_file -ne $mutationStartupConfigFile -or
                $specificProfileMutationRestore.restore_file -ne $specificProfileMutationBackupFile -or
                [int64]$specificProfileMutationRestore.profile_count -ne 1 -or
                -not $specificProfileMutationRestore.pre_restore_backup -or
                $specificProfileMutationRestore.pre_restore_backup.action -ne "restore-profile-mutation-backup" -or
                $specificProfileMutationRestore.pre_restore_backup.startup_config_file -ne $mutationStartupConfigFile -or
                [int64]$specificProfileMutationRestore.pre_restore_backup.byte_count -le 0
            ) {
                throw "Profile mutation backup specific restore did not report the expected restore metadata."
            }
            $specificProfileMutationRestoreText = $specificProfileMutationRestore | ConvertTo-Json -Depth 10
            if ($specificProfileMutationRestoreText -match "Release Secret Profile" -or $specificProfileMutationRestoreText -match "RELEASE_PROFILE_MUTATION_SECRET" -or $specificProfileMutationRestoreText -match "release-profile-mutation-secret" -or $specificProfileMutationRestoreText -match "Release Specific Secret Profile" -or $specificProfileMutationRestoreText -match "RELEASE_SPECIFIC_PROFILE_MUTATION_SECRET" -or $specificProfileMutationRestoreText -match "release-specific-profile-mutation-secret") {
                throw "Profile mutation backup specific restore output leaked startup config contents."
            }
            $specificProfileMutationRestoredStartupText = Get-Content -Raw -LiteralPath $mutationStartupConfigFile
            if ($specificProfileMutationRestoredStartupText -ne $specificProfileMutationBackupPayload) {
                throw "Profile mutation backup specific restore did not restore terminal.json from the requested rollback backup."
            }
...
'@ -Encoding ascii
    ...
    if (-not $SkipVisualSmoke) {
        ...
        if (-not $SkipShortcutVisualSmoke) {
            $shortcutVisualSmokeArgs = @(
                "-NoProfile",
                "-ExecutionPolicy", "Bypass",
                "-File", (Join-Path $repoRoot "script\zed-terminal-visual-smoke.ps1"),
                "-Binary", $ShortcutBinary,
                "-OutputDir", $shortcutVisualSmokeDir,
                "-StartupTimeoutSeconds", "$StartupTimeoutSeconds",
                "-CaptureDelaySeconds", "$CaptureDelaySeconds",
                "-VerifyShortcuts"
            )
            Invoke-Step "visual smoke shortcuts" {
                $shortcutVisualResult = Invoke-NativeCommandResult -FilePath "powershell" -Arguments $shortcutVisualSmokeArgs
